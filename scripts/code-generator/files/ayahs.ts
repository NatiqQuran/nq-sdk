import * as AyahsType from "./types/ayahs.ts";
...
export class AyahsController extends BaseController {
    ...
    /** PUT /ayahs/{uuid}/ */
    async update(
        uuid: string, data: AyahsType.AyahsUpdateRequestData, config?: RequestConfig
    ): Promise<AxiosResponse<AyahsType.AyahsUpdateResponseData>> {
        return await this.axiosPut(`/ayahs/${uuid}/`, data, config);
    }
    /** PATCH /ayahs/{uuid}/ */
    async partialUpdate(
        uuid: string, data: AyahsType.AyahsPartialupdateRequestData, config?: RequestConfig
