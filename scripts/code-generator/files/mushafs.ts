import * as MushafsType from "./types/mushafs.ts";
...
export class MushafsController extends BaseController {
    ...
    /** PUT /mushafs/{uuid}/ */
    async update(
        uuid: string, data: MushafsType.MushafsUpdateRequestData, config?: RequestConfig
    ): Promise<AxiosResponse<MushafsType.MushafsUpdateResponseData>> {
        return await this.axiosPut(`/mushafs/${uuid}/`, data, config);
    }
    /** PATCH /mushafs/{uuid}/ */
    async partialUpdate(
        uuid: string, data: MushafsType.MushafsPartialupdateRequestData, config?: RequestConfig
