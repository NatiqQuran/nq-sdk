import * as UsersType from "./types/users.ts";
...
export class UsersController extends BaseController {
    ...
    /** PUT /users/{uuid}/ */
    async update(
        uuid: string, data: UsersType.UsersUpdateRequestData, config?: RequestConfig
    ): Promise<AxiosResponse<UsersType.UsersUpdateResponseData>> {
        return await this.axiosPut(`/users/${uuid}/`, data, config);
    }
    /** PATCH /users/{uuid}/ */
    async partialUpdate(
        uuid: string, data: UsersType.UsersPartialupdateRequestData, config?: RequestConfig
