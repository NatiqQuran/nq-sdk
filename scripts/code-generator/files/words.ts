import * as WordsType from "./types/words.ts";
...
export class WordsController extends BaseController {
    ...
    /** PUT /words/{uuid}/ */
    async update(
        uuid: string, data: WordsType.WordsUpdateRequestData, config?: RequestConfig
    ): Promise<AxiosResponse<WordsType.WordsUpdateResponseData>> {
        return await this.axiosPut(`/words/${uuid}/`, data, config);
    }
    /** PATCH /words/{uuid}/ */
    async partialUpdate(
        uuid: string, data: WordsType.WordsPartialupdateRequestData, config?: RequestConfig
