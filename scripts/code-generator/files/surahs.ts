import * as SurahsType from "./types/surahs.ts";
...
export class SurahsController extends BaseController {
    ...
    /** PUT /surahs/{uuid}/ */
    async update(
        uuid: string, data: SurahsType.SurahsUpdateRequestData, config?: RequestConfig
    ): Promise<AxiosResponse<SurahsType.SurahsUpdateResponseData>> {
        return await this.axiosPut(`/surahs/${uuid}/`, data, config);
    }
    /** PATCH /surahs/{uuid}/ */
    async partialUpdate(
        uuid: string, data: SurahsType.SurahsPartialupdateRequestData, config?: RequestConfig
